...
class BaseDocumentLayout(models.TransientModel):
    ...
    _name = 'custom.base.document.layout'
    _description = 'Company Document Layout'

    @api.model
    def _default_report_footer(self):
        company = self.env.company
        footer_fields = [field for field in [company.phone, company.email, company.website, 'GST: '+company.vat] if isinstance(field, str) and len(field) > 0]
    ...
    external_letter_layout_id = fields.Many2one(readonly=False)
    ...
    def _compute_preview(self):
        ...
        for wizard in self:
            if wizard.report_layout_id:
                ...
                wizard.preview = ir_ui_view._render_template('ts_hr_letters.custom_report_invoice_wizard_preview', {
    ...
    def _onchange_company_id(self):
        for wizard in self:
            ...
            wizard_layout = wizard.env["report.layout"].search([
                ('view_id.key', '=', wizard.company_id.external_letter_layout_id.key)
            ])
    ...
    def _onchange_report_layout_id(self):
        for wizard in self:
            wizard.external_letter_layout_id = wizard.report_layout_id.view_id
    ...
    def _get_asset_style(self):
        ...
        :return:
        """
        company_styles = self.env['ir.qweb']._render('hr_letters.custom_styles_company_report',
                                                     {'company_ids': self,}, raise_if_not_found=False)
