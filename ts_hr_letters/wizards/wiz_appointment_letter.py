...
class WizSalaryStructure(models.TransientModel):
    ...
    # """Get default wage for appointment letter"""
        ...
        if active_id:
            ...
            if employee:
                active_contract = employee.contract_ids.filtered(
                    lambda contract: contract.state == 'open' and (not contract.date_end or contract.date_end >= fields.Date.today())
                )
                if active_contract:
                    return active_contract[0].wage
                else:
                    raise UserError('No Active Contract found with the Employee')    
        return ''
...
class WizOfferLetter(models.TransientModel):
    ...
    def action_offer_letter(self):
        template_id = self.env.ref('ts_hr_letters.email_template_offer_letter_applicant').id
        compose_form_id = self.env.ref('mail.email_compose_message_wizard_form').id
        template = self.env['mail.template'].browse(template_id)
        ...
        # raise UserError(self.application_id.id)
        ctx = {
            'default_model': 'wizard.offer.letter',
            'default_res_id': int(self.application_id.id),
