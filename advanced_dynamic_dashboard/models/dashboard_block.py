...
class DashboardBlock(models.Model):
    ...
    res_users_id = fields.Many2one("ir.model.fields",string="User Field")

    date_value = fields.Selection([('this_week', 'This Week'), ('last_week', 'Last Week'),
                                   ('this_month', 'This Month'), ('last_month', 'Last Month')], string="Date")
    user_value = fields.Many2one("res.users", string="User")
    ...
    def get_dashboard_vals(self, action_id, start_date=None, end_date=None, res_user=None):
        ...
        for rec in self.env['dashboard.block'].sudo().search(
            ...
            date_field = self.env['ir.model.fields'].browse(rec.date_field_id.id).name or False

            user_field = self.env['ir.model.fields'].browse(rec.res_users_id.id).name or False


            filter_list = [filter_item for filter_item in filter_list if not (
                    isinstance(filter_item, tuple) and filter_item[0] == user_field)]
            
            if not rec.user_value:
                if user_field and res_user != 'null' and res_user:
                    if not isinstance(res_user, int):
                        int_res = int(res_user)
                        filter_list.append((user_field, '=', int_res))
                    else:
                        filter_list.append((user_field, '=', res_user))
            elif user_field:
                filter_list.append((user_field, '=', rec.user_value.id))
            ...
            filter_list = [filter_item for filter_item in filter_list if not (
                    isinstance(filter_item, tuple) and filter_item[0] == date_field)]

            if not rec.date_value:
                if date_field and start_date and start_date != 'null':
                    start_date_obj = datetime.strptime(start_date, '%Y-%m-%d')
                    filter_list.append(
                        # ('create_date', '>=', start_date_obj.strftime('%Y-%m-%d'))
                        (date_field, '>=', start_date_obj.strftime('%Y-%m-%d')))

                if date_field and end_date and end_date != 'null':
                    end_date_obj = datetime.strptime(end_date, '%Y-%m-%d')
                    filter_list.append(
                        # ('create_date', '<=', end_date_obj.strftime('%Y-%m-%d'))
                        (date_field, '<=', end_date_obj.strftime('%Y-%m-%d')))

            elif date_field:
                today = datetime.today()
                start_date_obj = ''
                end_date_obj = ''
                if rec.date_value == 'this_week':
                    start_date_obj, end_date_obj = rec.get_week_start_end(today)
                    print("this_week")
                elif rec.date_value == 'last_week':
                    start_date_obj, end_date_obj = rec.get_week_start_end(today - timedelta(weeks=1))
                    print("last_week")
                elif rec.date_value == 'this_month':
                    start_date_obj, end_date_obj = rec.get_month_start_end(today)
                    print("this_month")
                elif rec.date_value == 'last_month':
                    last_month = today.replace(day=1) - timedelta(days=1)
                    start_date_obj, end_date_obj = rec.get_month_start_end(last_month)
                    print("last_month")
                filter_list.append((date_field, '>=', start_date_obj.strftime('%Y-%m-%d')))
                filter_list.append((date_field, '<=', end_date_obj.strftime('%Y-%m-%d')))
            # if date_field and start_date and start_date != 'null':
            #     start_date_obj = datetime.strptime(start_date, '%Y-%m-%d')
            #     filter_list.append(
            #         # ('create_date', '>=', start_date_obj.strftime('%Y-%m-%d'))
    ...
    def _onchange_model_id(self):
        self.group_by_id = self.measured_field_id = self.filter = self.operation = self.date_field_id = self.res_users_id = self.date_value = self.user_value = ''
    ...
    def check_user_group(self):
        print("check_user_group called")
        user = self.env.user
        if user.has_group('advanced_dynamic_dashboard.ts_dashboard_admin'):
            return True
        else:
            return False
