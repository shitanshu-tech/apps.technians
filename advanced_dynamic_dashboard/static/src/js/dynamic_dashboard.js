odoo.define('advanced_dynamic_dashboard.Dashboard', function (require) {
    ...
    var DynamicDashboard = AbstractAction.extend({
        ...
        events: {
            ...
            'change #default-date-filter': 'modifyDatesAndFilter',
            'change #start-date': '_onchangeFilter',
            'change #end-date': '_onchangeFilter',
            'change #user-names-list': '_onchangeFilter',
...
//                console.log("-------session_user-------",session_user.id,session_user.name);
            rpc.query({
            model: 'res.users',
            method: 'search_read',
            args: [[], ['id','name']],
...
//                        defaultOption.selected = true;
                    userDropdownElement.appendChild(defaultOption);
                    ...
                    users.forEach(user => {
                        ...
                        if(user.id === session.uid){option.selected = true;}
        ...
        fetch_data: function () {//Fetch data and call rpc query to create chart or tile. return block_ids
            ...
            var def1 = this._rpc({
                ...
                args: [[], this.action_id, , , session.uid]
            }).then(function (result) {
        ...
        switch_mode: function (ev) {//Function to change dashboard theme dark and light mode.
         ...
         modifyDatesAndFilter: function() {
            var date_filter = $('#default-date-filter').val();
            var start_date = $('#start-date').val();
            var end_date = $('#end-date').val();
            var today = new Date();
            var start_date_obj, end_date_obj;

            if (date_filter === 'This Week') {
                var currentDay = today.getDay();
                start_date_obj = new Date(today.setDate(today.getDate() - currentDay + (currentDay === 0 ? -6 : 1))); // Monday
                end_date_obj = new Date(today.setDate(start_date_obj.getDate() + 6)); // Sunday
            } else if (date_filter === 'Last Week') {
                var currentDay = today.getDay();
                start_date_obj = new Date(today.setDate(today.getDate() - currentDay - 6)); // Monday of last week
                end_date_obj = new Date(today.setDate(start_date_obj.getDate() + 6)); // Sunday of last week
            } else if (date_filter === 'This Month') {
                start_date_obj = new Date(today.getFullYear(), today.getMonth(), 1); // First day of current month
                end_date_obj = new Date(today.getFullYear(), today.getMonth() + 1, 0); // Last day of current month
            } else if (date_filter === 'Last Month') {
                start_date_obj = new Date(today.getFullYear(), today.getMonth() - 1, 1); // First day of last month
                end_date_obj = new Date(today.getFullYear(), today.getMonth(), 0); // Last day of last month
            } else {
                start_date_obj = start_date ? new Date(start_date) : null;
                end_date_obj = end_date ? new Date(end_date) : null;
            }
            ...
            this._onchangeFilter(); // Call the filter function with the modified dates
        },
        ...
        // Function for changing the filter
            ...
            var start_date = $('#start-date').val();
            var end_date = $('#end-date').val();
            var res_user = $('#user-names-list').val();
            var self = this;
                if (!start_date) {
                    start_date = "null";
                }
                ...
                this._rpc({
                    model: 'dashboard.block',
                    method: 'get_dashboard_vals',
                    args: [[], this.action_id, start_date, end_date, res_user],
                }).then(function (result) {
                    self.block_ids = result;
