...
class AccountCashBookReport(models.TransientModel):
    ...
    def get_html_report(self):
        ...
        report_ref = 'om_account_daily_reports.action_report_cash_book'
        report_obj = self.env.ref('om_account_daily_reports.action_report_cash_book').with_context(active_model='account.cashbook.report', active_ids=self.id)
        report_details = report_obj._render_qweb_html(report_ref, docids=None, data=data)[0]
        self.report_details = report_details
        return {
            'type': 'ir.actions.act_window',
            'view_mode': 'form',
            'view_id': self.env.ref('om_account_daily_reports.account_report_cashbook_view').id,
